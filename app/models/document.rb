class Document < ActiveRecord::Base
  ...
  def self.search_people(search_params)
    ...
    if !search_params[:date_birth_from].blank? && !search_params[:date_birth_to].blank? &&
       search_params[:date_birth_from] != 'yyyy'
    ...
    condition += "(extract(year FROM person_events.date_event) >= '#{date_birth_from}'
                  AND event_types.name = 'Birth') AND " unless date_birth_from.blank?
    condition += "(extract(year FROM person_events.date_event) <= '#{date_birth_to}'
                  AND event_types.name = 'Birth') AND " unless date_birth_to.blank?
